...
int main(int argc, char* argv[]) {
    ...
    bool use_network = (host && port && !uds_socket_path);
    bool use_uds = (uds_socket_path && !host && !port);


    if (!(use_network && use_uds)) {
        std::cerr << "ERROR: Specify either:\n" << "-h <hostname/IP> - p <port> OR -f <uds_socket_path>\n";
        return 1;
    }
