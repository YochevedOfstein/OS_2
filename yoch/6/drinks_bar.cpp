...
void processUDPCommandUNIX(int sock, const std::string& line, const sockaddr_un& cli_addr, socklen_t cli_len, unsigned long long& carbon, unsigned long long& oxygen, unsigned long long& hydrogen) {
    std::istringstream iss(line);
    std::string cmd;
    unsigned long long count;
    if (!(iss >> cmd) || cmd != "DELIVER") {
...
int main(int argc, char* argv[]) {
    ...
    int opt;
    while ((opt = getopt_long(argc, argv, "o:h:c:t:T:U:s:d", long_options, nullptr)) != -1) {
        switch (opt) {
            case 't':
            ...
            case 'U':
                ...
                break;
            case 's':
                std::cerr << "Stream path option is not implemented in this version.\n";
                break;
            case 'd':
                std::cerr << "Datagram path option is not implemented in this version.\n";
                break;
            case 'f':
                std::cerr << "Save file option is not implemented in this version.\n";
                break;
            default:
    ...
    // --- UDS stream setup ---
    int udsstream = socket(AF_UNIX, SOCK_STREAM, 0);

    sockaddr_un uds_stream_addr{};
    uds_stream_addr.sun_family = AF_UNIX;
    strncpy(uds_stream_addr.sun_path, "/tmp/uds_stream", sizeof(uds_stream_addr.sun_path) - 1);

    if (bind(udsstream , (sockaddr*)&uds_stream_addr, sizeof(uds_stream_addr)) < 0) {
        std::cerr << "Error binding UDS stream socket\n";
        return 1;
    }
    if (listen(udsstream , SOMAXCONN) < 0) {
        std::cerr << "Error listening on UDS stream socket\n";
        return 1;
    }
    setNonBlocking(udsstream);
    ...
    // --- UDS datagram setup ---
    int udsdatagram = socket(AF_UNIX, SOCK_DGRAM, 0);
    sockaddr_un uds_datagram_addr{};
    uds_datagram_addr.sun_family = AF_UNIX;
    strncpy(uds_datagram_addr.sun_path, "/tmp/uds_datagram", sizeof(uds_datagram_addr.sun_path) - 1);
    if (bind(udsdatagram, (sockaddr*)&uds_datagram_addr, sizeof(uds_datagram_addr)) < 0) {
        std::cerr << "Error binding UDS datagram socket\n";
        return 1;
    }
    setNonBlocking(udsdatagram);
    ...
    while (true) {
        FD_ZERO(&read_fds);
        FD_SET(STDIN_FILENO, &read_fds); // Standard keyboard input for commands
        FD_SET(listener, &read_fds); // TCP listener socket
        FD_SET(udpSock, &read_fds); // UDP socket
        FD_SET(udsstream, &read_fds); // UDS stream socket
        FD_SET(udsdatagram, &read_fds); // UDS datagram socket
        ...
        max_fd = std::max(max_fd, udsstream);
        max_fd = std::max(max_fd, udsdatagram);
        for(int fd : clients) {
        ...
        if (FD_ISSET(udsstream, &read_fds)) {
            sockaddr_un cli_addr{};
            socklen_t cli_len = sizeof(cli_addr);
            int client_fd = accept(udsstream, (sockaddr*)&cli_addr, &cli_len);
            if (client_fd < 0) {
        ...
        // Handle UDS datagram requests
        if (FD_ISSET(udsdatagram, &read_fds)) {
            char buf[READ_BUFFER];
            sockaddr_un cli_addr{};
            socklen_t cli_len = sizeof(cli_addr);
            ssize_t n = recvfrom(udsdatagram, buf, sizeof(buf), 0, (sockaddr*)&cli_addr, &cli_len);
            if (n < 0) {
            ...
            processUDPCommandUNIX(udsdatagram, cmd, cli_addr, cli_len, carbon, oxygen, hydrogen);
        }
